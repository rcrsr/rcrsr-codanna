...
- **Stale indexed paths warn instead of silently walking nothing:** a startup catch-up or `--force` rebuild now logs one `WARN` per tracked root that no longer exists on disk, pointing at `codanna index --prune-indexed-paths`; the `ReindexHasNothingToRebuild` refusal is reserved for the case where every tracked root is gone. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
