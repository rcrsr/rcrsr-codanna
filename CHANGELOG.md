...
- **Reindex serialization:** Concurrent full reindex runs (an MCP `reindex(force: true)` racing the watcher's overflow catch-up) are now serialized instead of racing; the loser is rejected immediately with a typed, retryable `REINDEX_IN_PROGRESS` error rather than tripping `clear()`'s `BatchInProgress` reject. The watcher's catch-up path no longer burns its bounded retry budget on such a rejection. ([#45](https://github.com/rcrsr/rcrsr-codanna/pull/45))
