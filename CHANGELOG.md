...
- **Watcher catch-up reindex is now incremental:** the overflow/rescan, backend-error, and `startup_catch_up` recovery paths now run a `CloneCurrent` staged build (hardlink-clone the current generation, then the ordinary hash-skip walk) instead of a forced full re-parse and re-embed of every indexed root, so a checkout stampede or short downtime costs roughly what the files that actually changed cost, not a full rebuild. Vanished files are still cleaned up by the walk. With no `indexed_paths` registered an episode now converges quietly instead of logging five `ERROR` lines and giving up. A full rebuild remains available on demand via `codanna index --force` / `reindex force:true`; no `catch_up_mode` knob was added. ([#95](https://github.com/rcrsr/rcrsr-codanna/issues/95)) ([#98](https://github.com/rcrsr/rcrsr-codanna/pull/98))
