...
- **`codanna serve --stop-all --include-unknown`:** `--stop-all` can now additionally sweep unregistered pids that still independently pass the `codanna serve` process-identity check (re-validated immediately before each is signaled), not just registered targets; these need no registry cleanup and are counted separately from registered targets in the summary line ("stopped N registered, M unknown"). `--include-rogue` is kept as a deprecated alias for one release cycle. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
- **`codanna index --prune-indexed-paths`:** drops tracked `indexed_paths` entries from the current generation's `index.meta` that are ghosts (no longer a directory on disk) or strays (outside `workspace_root` and not listed under `indexing.indexed_paths` in `settings.toml`). Configured out-of-tree roots are kept, since indexing a project from outside its directory is supported upstream. Metadata-only, like `--status`/`--gc`. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))

### Changed

- **`codanna ls`:** the SOURCE column value for an unregistered `codanna serve` pid discovered only by the process-table scan is now `unknown` (previously `rogue`). ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
- **`codanna ls`:** the STATUS column value for a registered proxy whose backing server is gone, dead, or unknown is now `detached` (previously `orphaned`); the proxy is still live and will revive a backing server on its next delegated call, which "orphaned" misread as dead.
- **`codanna index --status`:** the human-readable output is now a fixed-width table like `codanna ls` (`ID STATE AGE SIZE ERROR`) with humanized age (`23h 19m`) and size (`27.6 MiB`) instead of one `key=value` line per generation. `--status --json` is unchanged.
- **`codanna serve --stop <pid>` no longer requires registry membership:** a numeric `--stop <pid>` selector now accepts any pid that independently passes the `codanna serve` process-identity check, registered or not -- the previous `--include-rogue` opt-in for `--stop` is gone, since the identity check was always the real safety net, not registry membership. Stopping an unregistered pid this way prints a one-line note. A workspace-root path selector is unaffected: it still only resolves through the registry. `--include-rogue`/`--include-unknown` are now exclusively `--stop-all` flags (see above); using either with `--stop` is rejected at the CLI level. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
- **`codanna serve --kill-all` renamed to `--stop-all`:** matches the SIGTERM-by-default, `--force`-for-SIGKILL semantics of `--stop`, which `--kill-all` never implied. `--kill-all` is kept as a deprecated alias for one release cycle: it still works, but prints a deprecation notice to stderr. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
...
- **Stale indexed paths warn instead of silently walking nothing:** a startup catch-up or `--force` rebuild now logs one `WARN` per tracked root (from `settings.toml`'s `indexing.indexed_paths`) that no longer exists on disk, pointing at `codanna remove-dir <path>`; the `ReindexHasNothingToRebuild` refusal is reserved for the case where every tracked root is gone. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
- **Bounded, interrupting shutdown:** `--http`/`--https` now bound the post-signal watcher-handle joins to ~3s and exit regardless of whether they settle (registry/discovery cleanup already ran first), and the unified watcher's shutdown path now `abort()`s an in-flight catch-up reindex instead of waiting for it to finish -- safe because of index generations: an aborted build only orphans a half-built generation, reaped by GC on the next start. Stdio mode now also handles SIGTERM (dropping its serve lockfile) instead of relying on the default disposition. ([#97](https://github.com/rcrsr/rcrsr-codanna/pull/97))
- **Watcher catch-up reindex is now incremental:** the overflow/rescan, backend-error, and `startup_catch_up` recovery paths now run a `CloneCurrent` staged build (clone the current generation -- Tantivy segments hardlinked, the semantic store and `index.meta` byte-copied -- then the ordinary hash-skip walk) instead of a forced full re-parse and re-embed of every indexed root, so a checkout stampede or short downtime costs roughly what the files that actually changed cost, not a full rebuild. Vanished files inside a still-configured, still-present root are still cleaned up by the walk. If a previously-indexed root was removed from `indexing.indexed_paths` or its directory disappeared, and at least one other configured root remains valid, the catch-up automatically falls back to a one-time fresh rebuild instead of carrying that root's rows forward forever; with no configured root left valid at all, an episode now converges quietly (skipping the clone entirely) instead of logging five `ERROR` lines and giving up. A full rebuild remains available on demand via `codanna index --force` / `reindex force:true`; no `catch_up_mode` knob was added. ([#95](https://github.com/rcrsr/rcrsr-codanna/issues/95)) ([#98](https://github.com/rcrsr/rcrsr-codanna/pull/98))
- **Post-open semantic attach and enable-rule deduplicated:** the three copies of the "reattach semantic search + restore `indexed_paths` after opening a generation" block (`load_facade_impl`, `open_build` clone path, `swap_in`) now share `IndexFacade::attach_persisted_state`, and the two drifting copies of the "should this facade enable semantic search" rule (`main.rs` and `reindex_locked`) now share `IndexFacade::wants_semantic_search`. Behavior is unchanged; only the log wording at those sites is now uniform. ([#94](https://github.com/rcrsr/rcrsr-codanna/issues/94)) ([#98](https://github.com/rcrsr/rcrsr-codanna/pull/98))
...
- **`codanna ls` gains a VERSION column:** registered rows (server or proxy) show the `codanna` version the process recorded when it started; rogue rows (no live registry entry) instead show `same`, `other`, `deleted`, or `-`, from a pure path comparison between the rogue binary on disk and the `codanna ls` process's own binary -- never an exec or `--version` probe of the rogue process. ([#89](https://github.com/rcrsr/rcrsr-codanna/pull/89))
- **Server registry gains a `version` field:** `RegistryEntry` now records the `codanna` version (`env!("CARGO_PKG_VERSION")`) of the process that wrote it, backing the new `codanna ls` VERSION column. Defaults to an empty string via `#[serde(default)]` so entries written by older builds still deserialize. ([#89](https://github.com/rcrsr/rcrsr-codanna/pull/89))
...
- **`codanna ls` — top-level listing of every codanna process:** merges three sources into one read-only table (PID / KIND / SOURCE / PORT / SCHEME / STATUS / WORKSPACE): registered backing servers, registered proxies attributed to the backing server that shares their workspace root, and *rogue* `codanna serve` processes found by a full process-table scan that have no live registry entry (marked `rogue`, with workspace/port filled in best-effort from the process cwd, `.codanna/serve.json`, or a `--bind` argument). Rogue detection requires the executable basename to be `codanna` *and* an exact `serve` argument token, so unrelated processes that merely mention "codanna" never match. `ls` never reaps, signals, or rewrites registry files. Proxies (`codanna serve --proxy`) now write a lightweight `role: proxy` registry entry on connect and remove it on graceful exit; a crashed proxy's leftover entry is pruned by the existing staleness/`--reap` machinery. `RegistryEntry` gains a `role` field (`server` | `proxy`) that defaults to `server`, so entries written by older builds still parse. ([#82](https://github.com/rcrsr/rcrsr-codanna/issues/82)) ([#83](https://github.com/rcrsr/rcrsr-codanna/pull/83))
- **`codanna serve --stop <pid> --include-rogue`:** an explicit opt-in that lets `--stop` target a pid that is *not* in the per-user registry, as long as it still independently passes the `looks_like_codanna_serve` identity check — so a rogue server surfaced by `codanna ls` can actually be cleaned up. Without the flag, `--stop` keeps its registered-only contract unchanged; with it, a non-codanna pid is still refused. ([#82](https://github.com/rcrsr/rcrsr-codanna/issues/82)) ([#83](https://github.com/rcrsr/rcrsr-codanna/pull/83))
...
- **`codanna serve --list` deprecated in favor of `codanna ls`:** `--list` now prints a one-line deprecation notice to stderr, then delegates entirely to the same merge/table-building logic `codanna ls` owns, so the two commands always agree on their output. `--list` is kept for one release cycle for compatibility; prefer `codanna ls` going forward. ([#83](https://github.com/rcrsr/rcrsr-codanna/pull/83))

### Fixed

- **`discover_or_spawn` notices a dead spawned child instead of waiting out `spawn_timeout_ms`:** the spawn-wait loop now polls the spawned backing server's exit status alongside the health/record poll. If the child exits before the deadline (e.g. an index-format mismatch that makes `codanna serve --http` exit immediately with code 7), the proxy fails within milliseconds with a new `DiscoveryError::SpawnedProcessExited { code, stderr }` that carries the child's exit code and a bounded tail of its stderr, instead of a generic `SpawnTimeout` after the full 8 s wait with no hint of what went wrong. The child's stderr is redirected to a temp file (never a pipe), so a healthy backing server that outlives the proxy is never exposed to `EPIPE`. A slow-but-alive child still yields `SpawnTimeout` as before. ([#80](https://github.com/rcrsr/rcrsr-codanna/issues/80)) ([#83](https://github.com/rcrsr/rcrsr-codanna/pull/83))
...
- **`read_symbol`'s `source` now includes leading indentation, and any text preceding the declaration on its first line, for every indented symbol in every language** — see the matching `### Fixed` entry below. A consumer that assumed `source` started exactly at the declaration keyword must re-check how it parses `source`.

### Changed

- **Upstream base v0.13.1:** advanced the fork's upstream base from v0.12.0 to v0.13.1 and migrated the MCP stack from rmcp 2.x to 3.1.0; the crate version is now `0.13.1+rcrsr.3` (base advanced from v0.12.0, and the monotonic `+rcrsr` counter incremented from `.2` to `.3` for the fork-private fixes in this release — the base bump alone would have kept it at `.2`). The fork's proxy mode, reindex tool, watcher catch-up, `ignore_patterns`, and MCP-tool additions (`find_symbols`, `read_symbol`, `get_file_outline`) were reconciled with upstream's v0.13 incremental-refactor and envelope rework through a merge rather than a linear rebase, so both feature sets survive intact. Emission semantics and on-disk index format are inherited from upstream unchanged relative to v0.12.0; no forced rebuild beyond the v0.12→v0.13 upstream jump. ([#71](https://github.com/rcrsr/rcrsr-codanna/pull/71))

### Fixed

- **`Symbol.range` lines are now 1-indexed on every JSON surface — read this if you compensated with your own `+1`:** `range.start_line`/`range.end_line` on a `Symbol` were emitted 0-indexed at the JSON boundary while every other line field (`call_line`, outline/candidate `line`, `read_symbol`'s human-readable `location`) was already 1-indexed — an inconsistency the project's own 0.10.0 changelog entry incorrectly advertised as resolved ("range objects 0-indexed, scalar line fields 1-indexed, one convention on every surface"). `range.start_line`/`end_line` are now 1-indexed to match. **Columns are unchanged** — `start_column`/`end_column` remain 0-indexed exactly as stored; only the two line fields shifted. This touches the flattened/embedded `range` object wherever a `Symbol` backs a JSON payload: `find_symbol` success payloads, `get_calls`/`find_callers` rows (the `range` embedded via the flattened `CallRelation`/`CallerRelation`) **and their ambiguous-candidate lists**, `analyze_impact`'s symbol listings **and its ambiguous-candidate list**, `read_symbol`'s ambiguous-candidate list, and the `symbol` embedded in every `semantic_search_docs` / `semantic_search_with_context` hit (including the nested `context.relationships` symbol lists) — `get_calls`, `find_callers`, `analyze_impact`, and `read_symbol` all route their ambiguous outcome through the same `ambiguous_envelope(entity, name, candidates: Vec<Symbol>)` helper, so the shift applies equally to every candidate's `range` there. (`find_symbol` appears in this list for its success payload only: it never treats multiple matches as ambiguous, so it has no candidate list.) The surface is not limited to MCP: the `codanna retrieve describe|callers|calls|implementations|search --json` family emits the same `SymbolContext` payloads on success and shifts identically, as does any MCP client reading these envelopes over `codanna mcp <tool> --json` — with the exception of `read_symbol` and `get_file_outline`, whose `codanna mcp <tool> --json` CLI shortcut hard-codes text output (`src/cli/commands/mcp.rs`) and never reaches the JSON render chain, so the shift there is only visible to a real MCP client, not that CLI invocation. **If your integration already added its own `+1` to `range.start_line`/`end_line` to work around the old behavior, remove it now — leaving it in place will silently double-shift every reported line.** `schema_version` deliberately stays `1.0.0`: the envelope's structure (field names, nesting, status vocabulary) is unchanged; only a value inside an existing field changed, which this project's versioning does not treat as a schema break. Storage is unaffected — ranges remain 0-indexed on disk; the shift is applied only at (de)serialization on `Symbol::range`. ([#61](https://github.com/rcrsr/rcrsr-codanna/issues/61), [#70](https://github.com/rcrsr/rcrsr-codanna/pull/70))
- **`read_symbol` no longer strips leading indentation, or text before an export modifier, from `source`:** `extract_span` previously started slicing a symbol's first source line at `range.start_column` (the column of the declaration keyword itself, e.g. `class`), which silently dropped both leading whitespace and any modifier keyword preceding the keyword on the same line — most visibly `export ` before `export class Foo`. The first line is now sliced from byte 0, the same convention already used for every interior and trailing line of a multi-line span. The visible effect is broader than the export case that surfaced it: **leading indentation now appears on the first line of `source` for every indented symbol, in every language** — a Python method's `source` now begins with its leading spaces/tabs instead of starting at `def`. This is an intentional, accepted trade-off (documented on `extract_span`): a declaration that does not start its own line (e.g. two class declarations on one line) now absorbs the preceding text of that line too, rather than reconstructing "the start of the statement" with a language-specific heuristic. `read_symbol`'s derived `signature` field is unaffected — it is computed independently from parser byte offsets, not from `extract_span`. ([#62](https://github.com/rcrsr/rcrsr-codanna/issues/62), [#70](https://github.com/rcrsr/rcrsr-codanna/pull/70))
- **`serve --proxy` revives a dead backing-server connection instead of failing every call until a manual restart:** a delegated MCP call (`tools/call`, `tools/list`, etc.) that observes a dead transport (`ServiceError::TransportClosed` or `TransportSend`) — the backing server crashed, was killed, or its process exited — now triggers exactly one reconnect attempt before the call is retried once against the fresh connection, reusing `discover_or_spawn` verbatim as the reconnect primitive so a revive re-decides against a fresh discovery record, handles a stale PID, and single-flights across processes exactly the way startup discovery already does. Limits, by design: **one retry only** — a second dead-transport failure after the revive is returned as an error rather than looped or backed off; **dead-transport only** — a slow-but-alive backing server (`ServiceError::Timeout`, or the proxy's own call timeout) never triggers a revive, since replacing a merely-slow connection would be wrong; **single-flight per proxy process, on failure too** — concurrent delegated calls that all observe the same dead generation share one reconnect dial instead of racing multiple spawns, gated by a generation counter over the connection slot; a dial that fails still advances the generation and caches the error for that round, so same-generation waiters get the shared error back instead of each re-dialing (previously only a successful dial was shared, so N waiters against a slow-failing `auto_spawn` paid N full spawn-and-health-check timeouts back to back). If the workspace's discovery record has `auto_spawn = false` and no server is running, the revive does not fall back to spawning one: it returns an actionable error naming the workspace root and the underlying discovery failure, rather than reviving. ([#63](https://github.com/rcrsr/rcrsr-codanna/issues/63), [#70](https://github.com/rcrsr/rcrsr-codanna/pull/70))
- **`serve --proxy` discovery verifies a backing `--http` server's identity via a per-launch token, not just a live PID:** `.codanna/serve.json` previously trusted any record whose PID was alive and whose cmdline looked like `codanna serve`, which a same-user local process could satisfy by racing a plausible-looking record into place ahead of the real server. Each `--http` backing server now writes a fresh random token into the record and echoes it back from `/health`; discovery re-verifies that token on every read — the fast path and the spawn-wait loop alike, so it also applies on every `serve --proxy` revive, not just at proxy startup. A record whose token doesn't match, or that has none at all (written by an older codanna binary, pre-token), is rejected exactly like "no live server": discovery falls through to spawning a fresh one on a new port. `--https` records are deliberately not probed this way — their identity already rests on the pinned certificate checked at the real connection site, and probing them here would fall through to spawning `--http`, silently downgrading a rejected TLS connection to plaintext instead of surfacing the cert mismatch. One consequence worth knowing: a backing server started by an older, pre-token binary is no longer discovered — the proxy ignores it and spawns its own on a new port, at the cost of one extra resident process until the stale one is stopped or idles out. ([#63](https://github.com/rcrsr/rcrsr-codanna/issues/63), [#70](https://github.com/rcrsr/rcrsr-codanna/pull/70))
...
- **Watcher responsiveness on directory creation:** handling a created directory walked the same subtree twice (once for directories, once for files), synchronously and inline on the async runtime worker while holding the index read lock, with no debounce; it is now a single combined walk that runs off the runtime on a blocking thread, with the read lock released first. That walk is not scoped to the created subtree — it covers the whole owning indexed root, deliberately, to anchor the ignore chain there — so created-directory paths are now debounced and coalesced by owning root as well: a burst costs one walk per root per settled batch instead of one per created directory, regardless of nesting or sibling spread. Deferring the walk past the burst also means the snapshot sees more of the newly-materialized tree, and a catch-up reindex can no longer fire while directory discovery is still queued. A `git checkout`, `npm install`, or archive extraction under a watched root no longer stalls the watcher. Discovery results are unchanged; only the walk count and where it runs changed. ([#48](https://github.com/rcrsr/rcrsr-codanna/pull/48), [#51](https://github.com/rcrsr/rcrsr-codanna/pull/51))
...
- **Rust #[cfg(test)] caller classification:** Callers in #[cfg(test)] modules are now correctly classified as test code; existing indexes are corrected at query time without reindex. ([#52](https://github.com/rcrsr/rcrsr-codanna/pull/52))
- **Fork install path:** the release pipeline named its assets from the raw `Cargo.toml` version, so the fork's `+rcrsr.N` build metadata landed in the filename — a character GitHub rewrites on upload, leaving every archive under a name neither the installer nor `cargo binstall --git` would ever request. Asset filenames and the archive's inner directory now carry a sanitized version (`+` → `-`), while the git tag and `codanna --version` keep the `+` unchanged. The four binstall `pkg-url`/`bin-dir` overrides carry the sanitized version as a literal, since binstall's templating has no transform syntax to derive it; `tests/binstall_metadata_tests.rs` guards those literals against drift. Manifest platform extraction switched from a `-`-separator regex to an exact prefix strip, which the sanitized version would otherwise have broken silently. The installer scripts, inherited from upstream verbatim, resolved releases from `bartolli/codanna` and now point at this fork; `install.sh` additionally prefers the native arm64 build when run from a Rosetta-translated shell. A tag push is now gated on the full test suite instead of running it twice in parallel with neither run gating, `workflow_dispatch` exercises the pipeline as a dry run, and release notes come from the matching `CHANGELOG.md` section rather than an auto-generated commit shortlog. ([#56](https://github.com/rcrsr/rcrsr-codanna/pull/56))
