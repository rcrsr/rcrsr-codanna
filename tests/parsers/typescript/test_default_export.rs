...
mod tests {
    ...
    fn test_inline_default_export_function_extracted() {
        let code = r#"
export default function BrokenPage() { return sharedHelper(1); }
"#;
        ...
        assert!(
            names.contains(&"BrokenPage"),
            "inline `export default function` must extract. Got: {names:?}"
        );
        let sym = symbols
            .iter()
            .find(|s| s.name.as_ref() == "BrokenPage")
            .unwrap();
        assert_eq!(
            sym.visibility,
            Visibility::Public,
...
"#;
        ...
        assert!(
            ...
            "members of a default-exported class must extract. Got: {names:?}"
        );
    }

    #[test]
    fn test_split_default_export_unchanged() {
        // The split form worked before the inline fix; the recursion change
        // must not duplicate it or extract phantom symbols from the bare
        // identifier re-export.
        let code = r#"
...
"#;
        ...
        assert!(
            calls
                .iter()
                .any(|(from, to, _)| *from == "BrokenPage" && *to == "sharedHelper"),
            "call inside inline default export must attribute to it. Got: {calls:?}"
        );
        let broken_calls = calls
            .iter()
            .filter(|(from, to, _)| *from == "BrokenPage" && *to == "sharedHelper")
            .count();
        assert_eq!(
            broken_calls, 1,
