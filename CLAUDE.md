...
- **Catch-up rubric (deciding what to keep when advancing the base).** Default rule: **keep** fork-private code that adds a capability the base still lacks; **abandon** fork-private code once the base ships its own solution to the *same problem* — even if ours was fine — and adopt upstream's. This keeps the fork shrinking toward `upstream + a small additive delta` instead of accreting parallel implementations that re-conflict every rebase. Sort each fork-private change into one of three buckets:
  - **KEEP** — additive; the base still lacks the capability. Carry forward unchanged (e.g. proxy mode, watch-queue overflow catch-up, the reindex gate, scoped document collections).
  - **ABANDON** — the base converged *and* its version is **verified to cover the same case(s) ours does**. Delete ours, adopt theirs.
  - **UNION / SPLIT** — same problem, but the base solved a different slice. Adopt the base's part; keep only the fork delta the base does not deliver (e.g. take upstream's envelope stream markers, keep the fork's `ambiguous` constructor).
  - **EXTEND (superset) — rebuild our extra capability *on top of* the base, don't abandon it.** When the base's new feature is a **subset** of what our implementation already offers, the goal is to *incrementally improve the base*, not discard our work: adopt the base's implementation as the foundation and re-base our additional capabilities onto it, so we ride upstream's version where they overlap and keep our surplus on top. Example: upstream added a force-reindex request handler; our `reindex` MCP tool is a superset (discoverable `#[tool]` + `codanna mcp reindex` CLI + `documents` reindex), so we re-point the tool body at upstream's handler and keep the tool. Do this **when and where it makes sense** — when the base's foundation genuinely subsumes our lower layer and re-basing shrinks our delta. When the base's implementation is weaker or differently-shaped enough that adopting it costs more than it saves, keep ours intact (KEEP) and note why. Prefer **upstreaming** the surplus capability when it's general enough that upstream would take it — that retires the divergence entirely.
