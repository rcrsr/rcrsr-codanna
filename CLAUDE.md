...
- Fork-specific features currently live in: `serve --proxy` mode (`src/mcp/proxy.rs`, `src/serve_discovery.rs`, `src/serve_tls.rs`), the `reindex` MCP tool (`src/mcp/tools/admin.rs`), catch-up reindex on watch-queue overflow (`src/watcher/`), and index generations with build-and-publish (`src/storage/generation/`, `src/storage/persistence.rs`).
