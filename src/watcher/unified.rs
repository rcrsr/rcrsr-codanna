...
impl UnifiedWatcherBuilder {
    ...
    /// mid-handler) once this token is cancelled. See the field doc on
    /// [`UnifiedWatcher::cancellation_token`] for why this replaces wrapping
    /// `watch()` in an external `select!` against cancellation.
